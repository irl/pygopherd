...
def log(exception, protocol = None, handler = None):
    ...
    exceptionclass = re.search("[^.]+$", str(exception.__class__.__name__)).group(0)
    if protocol:
        protostr = re.search("[^.]+$", str(protocol.__class__.__name__)).group(0)
        ipaddr = protocol.requesthandler.client_address[0]
    if handler:
        handlerstr = re.search("[^.]+$", str(handler.__class__.__name__)).group(0)
